/*
  ...
  This function fetches and parses RSS feeds, then stores articles in the database.
  It's designed to be called on a schedule (every hour) to keep news updated.
*/
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Processed ${sources.length} sources, added ${totalArticles} new articles`,
        totalArticles,
    ...
    // Robust error handling - ensure we always have a string message
